    ...
    const fetchPublications = async () => {
      try {
        ...
        Swal.fire({
          ...
          position: 'top-end',
          icon: 'error',
          title: 'Impossible de récupérer les publications',
  ...
  const submitPublication = async () => {
    ...
    try {
      ...
      const res = await axios.post('/api/publication', payload)
       ...
       // 🔹 Transformation pour Publication.vue
        const newPub = {
          id: res.data.id,
          title: pubTypes.value.find(p => p.id === selectedPubType.value)?.name || 'Type inconnu', // anciennement description
          ...
          category_name: categories.value.find(c => c.id === selectedCategory.value)?.name || 'Catégorie inconnue', // anciennement pub_type_name
      ...
      form.value = { price:'', bathroom:'', surface:'', advance:'', deposit:'', description:'', visit:'', sale_or_rent:'', status:'', images: [] }
      previewImages.value = []
...
<style scoped>
  ...
  ::v-deep(.vs__clear) {
    fill: #dc3545; /* rouge bootstrap par défaut */
    cursor: pointer;
  }

  ::v-deep(.vs__clear:hover) {
    fill: #a71d2a; /* rouge un peu plus foncé au survol */
  }

  /* Couleur de la flèche (dropdown icon) */
  ::v-deep(.vs__open-indicator) {
    fill: #ffffff; /* vert bootstrap */
    transition: transform 0.2s ease, fill 0.2s ease;
  }

  /* Flèche quand le menu est ouvert (elle se retourne) */
  ::v-deep(.vs--open .vs__open-indicator) {
    fill: #f1f4f8; /* change de couleur quand ouvert */
